...
namespace ExcelOperate
{
    public class ExcelHelper
    {
        ...
        public static List<T> ReadExcel<T>(string[] columns, Stream excelSteam, string excelType)
        {
            var result = new List<T>();
            try
            {
                //创建空的工作薄
                IWorkbook workbooke = null;
                //检查格式以及Excel版本
                if (excelType == ".xlsx")
                {
                    //空的文件博等于XSSFWorkbook读取文件流
                    workbooke = new XSSFWorkbook(excelSteam);
                }
                else
                {
                    //如果是.xls就是HSSFWorkbook读取文件流  
                    workbooke = new HSSFWorkbook(excelSteam);
                }
                //工作簿是否大于零？如果大于零获取第一个工作薄的内容，否则空值
                ISheet sheet = workbooke.NumberOfSheets > 0 ? workbooke.GetSheetAt(0) : null;
                if (sheet == null)
                {
                    //工作薄内容为空 
                    return result;
                ...
                for (int i = 0; i < sheet.LastRowNum; i++)
                {
                    ...
                    if (row != null)
                    {
                        ...
                        {
                            //pro 得到的列头的类型
                            var pro = type.GetProperty(columns[j]);
                            if (pro != null)
                            {
                                //声明一个弱类型
                                dynamic value;
                                ...
                                switch (cell.CellType)
                                {
                                    ...
                                    case CellType.Formula:
                                        HSSFFormulaEvaluator e = new HSSFFormulaEvaluator(workbooke);
        ...
        public static List<T> DataTableToList<T>(DataTable dt) where T : class, new()
        {
            ...
            {
                var ss= item.GetCustomAttribute(typeof(DisplayNameAttribute), true);
                prolist.Add(item);
                dt.Columns[((DisplayNameAttribute)ss).DisplayName].ColumnName = item.Name;
            }

           
            //获得TResult 的所有的Public 属性 并找出TResult属性和DataTable的列名称相同的属性(PropertyInfo) 并加入到属性列表 
            //System.Array.ForEach(t.GetProperties(), p =>
            //{
            //    if (dt.Columns.IndexOf(p.Name) != -1) prolist.Add(p);
            //});
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                foreach (var item in prolist)
                {
                    ...
                    {
                        var ss = row[item.Name];
                        item.SetValue(ob, row[item.Name].ToString(), null);
                    }
